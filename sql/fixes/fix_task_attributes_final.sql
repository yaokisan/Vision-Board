-- レイヤーベースで業務ノードの属性を修正するSQLスクリプト（完全型キャスト修正版）
...
-- 5. 修正後の確認（すべての型キャストを修正）
SELECT 'AFTER FIX - TASKS:' as section;
SELECT 
  t.name as task_name,
  l.name as layer_name,
  b.name as business_name,
  CASE WHEN t.attribute IS NULL THEN 'NULL (company)' ELSE t.attribute END as task_attribute,
  CASE WHEN b.attribute IS NULL THEN 'NULL (company)' ELSE b.attribute::text END as business_attribute,
  CASE 
    WHEN t.attribute IS NOT NULL AND t.attribute = b.id::text THEN 'MATCH'
    WHEN t.attribute IS NULL AND b.id IS NULL THEN 'OK (company)'
    ELSE 'CHECK'
  END as status
FROM tasks t
LEFT JOIN layers l ON t.layer_id = l.id
LEFT JOIN businesses b ON b.id::text = t.attribute
ORDER BY l.name, t.name;
...
SELECT 
  ...
  t.name as task_name,
  CASE WHEN e.attribute IS NULL THEN 'NULL (company)' ELSE e.attribute END as executor_attribute,
  CASE WHEN t.attribute IS NULL THEN 'NULL (company)' ELSE t.attribute END as task_attribute,
  CASE 
    WHEN (e.attribute IS NULL AND t.attribute IS NULL) THEN 'MATCH (company)'
    WHEN (e.attribute IS NOT NULL AND t.attribute IS NOT NULL AND e.attribute = t.attribute) THEN 'MATCH'
    ELSE 'MISMATCH'
  END as status
FROM executors e
...
-- 8. レイヤー別の詳細確認
SELECT 'LAYER DETAILS:' as section;
SELECT 
  l.name as layer_name,
  'Business: ' || COALESCE(b.name, 'None') as business_info,
  'Tasks: ' || STRING_AGG(DISTINCT t.name, ', ') as task_info
FROM layers l
LEFT JOIN businesses b ON b.layer_id = l.id
LEFT JOIN tasks t ON t.layer_id = l.id
